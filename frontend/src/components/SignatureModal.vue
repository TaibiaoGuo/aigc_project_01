            ...
            if (oldCtx && oldCanvas) {
                ...
                if (currentBrush.value) {
                    currentBrush.value.updateOptions({
                        ...
                        maxLineWidth: drawingSettings.maxLineWidth,
                        pressureSensitivity:
                            drawingSettings.pressureSensitivity,
                    });
                }
            }
        }
...
<template>
    ...
    >
        <div
            class="bg-gradient-to-br rounded-2xl shadow-xl p-5 flex flex-col w-[75vw] h-[80vh]"
        >
            <div class="flex justify-between items-center">
                <div class="font-bold text-2xl text-gray-800 drop-shadow-md">
                    请签上你的大名
                </div>
                <div
                    class="w-9 h-9 rounded-full bg-white bg-opacity-20 border-2 border-white flex justify-center items-center cursor-pointer text-2xl text-gray-800 transition-all hover:bg-opacity-40 hover:scale-105"
                    @click="handleClose"
                >
                    ×
                </div>
            ...
            <div class="flex-1 flex justify-center items-center w-full my-4">
                <div
                    class="w-full h-full bg-white rounded-xl border-[5px] border-white shadow-md"
                >
                    <canvas
                        ref="canvasRef"
            ...
            <div class="flex justify-center gap-5 mt-2">
                <button
                    class="px-6 py-2.5 rounded-full text-base font-medium cursor-pointer transition-all bg-gray-100 text-gray-700 hover:-translate-y-1 hover:shadow-md active:translate-y-0.5 active:shadow-sm"
                    @click="clearCanvas"
                >
                    重写
                </button>
                <button
                    class="px-6 py-2.5 rounded-full text-base font-medium cursor-pointer transition-all bg-blue-500 text-white hover:-translate-y-1 hover:shadow-md active:translate-y-0.5 active:shadow-sm"
                    @click="handleConfirm"
                >
                    确定
                </button>
                <button
                    class="px-6 py-2.5 rounded-full text-base font-medium cursor-pointer transition-all bg-gray-200 text-gray-600 hover:-translate-y-1 hover:shadow-md active:translate-y-0.5 active:shadow-sm"
                    @click="handleClose"
                >
                    返回
                </button>
