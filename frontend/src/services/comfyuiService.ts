...
// 上传草图到后端
export async function uploadSketch(imageData: string, styleName: string = 'realistic', sessionId?: string): Promise<SessionInfo> {
  try {
    // 将 base64 转换为 Blob
...
// 创建WebSocket连接
export function createWebSocketConnection(sessionId: string, callbacks: {
  onOpen?: () => void;
...
// 轮询任务状态
export async function pollTaskStatus(taskId: string, onProgress?: (progress: number, total: number) => void): Promise<string> {
  try {
